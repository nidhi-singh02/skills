...
Concrete commands for each phase. Read this when you're about to build a Short. Every `<PLACEHOLDER>` (crop windows, segment times, switch points) is footage-specific — sample frames to find yours (see the placeholder list under Reframe filters); the *structure* of each command is what's stable.
...
## Reframe filters
All output 1080x1920. Sample frames first to find every placeholder: the face crop, the face->screen switch (= when the screen CONTENT appears, not when it's mentioned), and the PIP location.

Placeholders (derive from YOUR footage by sampling frames):
- `<FW>:<FH>:<FX>` — face crop: a 9:16 window on the face, so `<FW> = <FH>*9/16` (for 1080p source: `<FH>=1080`, `<FW>=608`), `<FX>` = x-offset centering the face.
- `<SW>:<SH>:<SX>:<SY>` — screen crop: a 9:16 window inside the screen-share region; `<SW> = <SH>*9/16`.
- `<TRAVEL>` — vertical scroll distance = `sourceHeight - <SH>` (how far the window can pan down).
- `<PIPX>:<PIPY>`, `<PIPSIZE>` — square crop around the face for the bubble; `<BUBY>` = bubble y-position on the 1920 canvas (bottom area, clear of subtitles).

**Face segment (crop-to-fill on face):**
```
[0:v]trim=A0:A1,setpts=PTS-STARTPTS,crop=<FW>:<FH>:<FX>:0,scale=1080:1920[v1]
```

**Screen segment (MUST slow-scroll — a static screen reads dead):**
```
[0:v]trim=B0:B1,setpts=PTS-STARTPTS,crop=<SW>:<SH>:<SX>:<TRAVEL>*t/<SEGDUR>,scale=1080:1920[v2]
```
The pan reveals top->bottom of the (static) viewport = looks like scrolling.
...
[a]crop=<SW>:<SH>:<SX>:<TRAVEL>*t/<SEGDUR>,scale=1080:1920[base];
[b]crop=<PIPSIZE>:<PIPSIZE>:<PIPX>:<PIPY>,scale=420:420[bub];
[base][bub]overlay=x=(W-w)/2:y=<BUBY>[c]
```
Optional border (use your brand color): `drawbox=x=(1080-420)/2:y=<BUBY>:w=420:h=420:color=<BRAND>:t=5`. Frame the bubble FULL-FACE with headroom — cropping too low/tight cuts the forehead.
...
```
[0:v]trim=<CLEAN0>:B1,setpts=PTS-STARTPTS,crop=<SW>:<SH>:<SX>:<TRAVEL>*t/<DUR>,scale=1080:1920,tpad=start_duration=0.7:start_mode=clone,setpts=PTS-STARTPTS[v2]
```
